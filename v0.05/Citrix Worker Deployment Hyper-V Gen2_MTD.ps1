...
<#
.SYNOPSIS
    Citrix Worker Deployment Hyper-V Gen2 - MTD
.Description
    Fuegt einem Worker eine System HDD hinzu und stellt die Boot Reihenfolge um.
.NOTES
    Author: 
    Patrik Horn (PHo)
    Link:	
...
#region
    ...
    Foreach ($VM in $SelectVMs){
        ...
        Try
        {
            Add-VMHardDiskDrive -ComputerName $VM_Host -VMName $VM_Name -ControllerType SCSI -ControllerNumber 0 -ControllerLocation 0 -Path "$VM_Path\$VM_Name\Virtual Hard Disks\$GL_SystemDisk_Name.vhdx"  | Out-Null
            $Msg = "Successfully"
            Write-Host $Msg -ForegroundColor Green
        ...
        Try
        {
            Add-VMScsiController -ComputerName $VM_Host -VMName $VM_Name
            Add-VMDvdDrive -ComputerName $VM_Host -VMName $VM_Name -ControllerNumber 1 -ControllerLocation 0
            $Msg = "Successfully"
            Write-Host $Msg -ForegroundColor Green
            $Tmp_Result_Add_SystemDisk = $Msg
        }
        ...
        Try
        {
            $VM_BootOrder_network = Get-VMNetworkAdapter -ComputerName $VM_Host -VMName $VM_Name | Where-Object { $_.Name -like $GL_NIC_Name_SRV }
            $VM_BootOrder_HDD = Get-VMHardDiskDrive -ComputerName $VM_Host -VMName $VM_Name -ControllerType SCSI -ControllerNumber 0 -ControllerLocation 0
            $VM_BootOrder_DVD = Get-VMDvdDrive -ComputerName $VM_Host -VMName $VM_Name -ControllerNumber 1 -ControllerLocation 0
            Set-VMFirmware -ComputerName $VM_Host -VMName $VM_Name -BootOrder $VM_BootOrder_network, $VM_BootOrder_DVD, $VM_BootOrder_HDD
            $Msg = "Successfully"
            Write-Host $Msg -ForegroundColor Green
        ...
        $Tmp_Result_Summary = @{
            ...
            "Add System Disk" = $Tmp_Result_Add_SystemDiskd
